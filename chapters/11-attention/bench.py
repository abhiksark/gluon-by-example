# chapters/11-attention/bench.py
"""Benchmarks FlashAttention forward: torch SDPA vs Triton FA.
...
Usage: python chapters/11-attention/bench.py
"""
...
import gluon_by_example.triton_impl.attention as ta
...
def main() -> None:
    ...
    csv_path = results_dir / f"attention-{slug}.csv"
    with open(csv_path, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["n", "provider", "tflops"])
        writer.writeheader()
        writer.writerows(rows)

    png_path = charts_dir / f"attention-{slug}.png"
