# chapters/10-normalization-gluon/bench.py
"""Benchmarks LayerNorm: torch vs Gluon forward and Gluon backward (partial only).

Fixed M=4096 rows, sweeping row width N. Writes CSV + chart.

The Gluon backward uses only the two-stage grouped-partial reduction (no
atomic floor). The backward provider moves roughly 3x the bytes of the
forward, so its effective-bandwidth number is not directly comparable to
the forward rows.

Usage: python chapters/10-normalization-gluon/bench.py
"""
...
from gluon_by_example.gluon_impl.normalization import layer_norm as gn_layer_norm
...
def main() -> None:
    """Runs Gluon LayerNorm benchmark across providers and row widths, writes CSV + PNG."""
    require_idle_gpu()
    rows = []
    for n in COLS:
        ...
        print(f"N={n:>6}  {'torch':<22} {gbps:8.1f} GB/s")

        # Gluon forward.
        ms = triton.testing.do_bench(
            lambda: gn_layer_norm(x_fwd, w_fwd, b_fwd)
        )
        gbps = 2 * M * n * x_fwd.element_size() / ms * 1e-6
        rows.append({"n": n, "provider": "gluon", "gbps": round(gbps, 2)})
        print(f"N={n:>6}  {'gluon':<22} {gbps:8.1f} GB/s")

        # Gluon backward (two-stage partial only; no atomic floor in Gluon).
        ...
        def fwd_bwd_partial():
            ...
            gn_layer_norm(x_bwd, w_bwd, b_bwd).backward(dy)

        ms = triton.testing.do_bench(fwd_bwd_partial)
        # Backward moves ~3x the bytes of the forward; this number is not
        # directly comparable to the forward rows.
        gbps = 2 * M * n * x_bwd.element_size() / ms * 1e-6
        rows.append({"n": n, "provider": "gluon-dw-partial", "gbps": round(gbps, 2)})
        print(f"N={n:>6}  {'gluon-dw-partial':<22} {gbps:8.1f} GB/s")
    ...
    csv_path = results_dir / f"normalization-gluon-{slug}.csv"
    with open(csv_path, "w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=["n", "provider", "gbps"])
        writer.writeheader()
        writer.writerows(rows)

    png_path = charts_dir / f"normalization-gluon-{slug}.png"
    make_bandwidth_chart(
        csv_path,
        png_path,
        title=f"LayerNorm Gluon, {M} rows -- {torch.cuda.get_device_name(0)}",
