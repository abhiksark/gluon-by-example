...
an explicit cp.async pipeline: a STAGES-deep shared-memory ring buffer prefetches
each K-tile's global->shared copies (STAGES-1) iterations ahead, so the next
tile's loads overlap the current tile's mma_v2 instead of stalling on them. At
the Triton-matched footprint (BLOCK_K=32, STAGES=3, num_warps=4) it claws back
the latency the unpipelined floor leaves on the table and beats the floor at
moderate N, but a codegen gap keeps it short of the Triton compiler's autotuned
GEMM. The full story — footprint vs occupancy, why STAGES=3 is the apples-to-
apples match, and where the residual gap comes from — is in chapter 5.
"""
...
def matmul(a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
    ...
    _pipe_matmul_kernel[grid](
        a, b, c, M, N, K, _BLOCK_M, _BLOCK_N, _BLOCK_K, _STAGES,
